...
const GridMotion = ({ items = [], gradientColor = 'black' }) => {
  ...
  useEffect(() => {
    ...
    const updateMotion = () => {
      ...
      const baseDuration = 4; // Slower for smooth continuous motion
      const inertiaFactors = [0.6, 0.4, 0.3, 0.2];

      rowRefs.current.forEach((row, index) => {
        if (row) {
          const direction = index % 2 === 0 ? 1 : -1;
          // Create a seamless back-and-forth animation
          gsap.to(row, {
            x: maxMoveAmount * direction,
            duration: baseDuration + inertiaFactors[index % inertiaFactors.length],
            ease: 'sine.inOut',
            repeat: -1, // Infinite loop
            yoyo: true, // Reverse animation for smooth back-and-forth
            overwrite: 'auto',
          });
        }
      });
    ...
    updateMotion(); // Start animation immediately

    return () => {
      gsap.killTweensOf(rowRefs.current); // Clean up animations
    };
  }, []);
